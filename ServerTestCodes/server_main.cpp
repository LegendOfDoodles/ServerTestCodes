...
void DisconnectPlayer(int id)
{
	...
	{
		if (g_clients[i].m_isconnected == true && i != id) SendPacket(i, &p);
	}
...
void ProcessPacket(int id, char *packet)
{
	...
	{
	case CS_ACTION:
		cout << "[" << id << "] Clients Change Animation\n";
		break;
	case CS_MOVE_PLAYER:
		x = g_clients[id].m_x;
		y = g_clients[id].m_y;
	...
	pos_packet.y = y;
	for (int i = 0; i < MAX_USER; ++i) {
		if(true == g_clients[i].m_isconnected) SendPacket(i, &pos_packet);
	}
...
void worker_thread()
{
	while (true)
	{
		...
		if (FALSE == ret) {
			cout << "Client [" << key << "] is Disconnected\n";
...
void accept_thread()	//새로 접속해 오는 클라이언트를 IOCP로 넘기는 역할
{
	...
	while (true)
	{
		...
		//연결되어 있는 모든 클라이언트에게 보냄
		for (int i = 0; i < MAX_USER; ++i) {
			if(g_clients[i].m_isconnected == true)
				SendPacket(id, &p);
		}
		for (int i = 0; i < MAX_USER; ++i) {
			if (g_clients[i].m_isconnected == false) continue; //접속하지 않은 클라이언트는 건너뜀
			if (i == id) continue; //나 자신한테는 한번만
